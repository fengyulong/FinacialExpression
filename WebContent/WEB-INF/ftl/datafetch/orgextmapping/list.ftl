...
<body>
			...
			<div class="easyui-layout" data-options="fit:true">
				...
				<div data-options="region:'center'">
					<div class="easyui-layout" data-options="fit:true">
						...
						<div data-options="region:'center'">
							<form id="orgmapping_form" action="${path}/datafetch/orgextmapping/add" method="post">
	...
	$(function() {
		...
		orgMappingGrid = $('#orgmapping_grid').datagrid({
			url : '${path}/datafetch/orgextmapping/list',
			singleSelect:true,
			rownumbers : true,
			striped : true,
			fit : true,
	...
	function add(){
		...
		if(node){
			...
			$('#orgmapping_form').form({
				url : '${path}/datafetch/orgextmapping/add',
	...
	function edit(){
		...
		setToolBar(false,false,true,false);
		$('#orgmapping_form').form({
			url : '${path}/datafetch/orgextmapping/edit',
			success : function(data){
				if(data == 'success'){
		...
		if(row){
			parent.$.messager.confirm('提示','删除后无法恢复，您确定要删除此机构及其下级？',function(data){
				if(data){
					$.ajax({
						type : 'post',
						url : '${path}/datafetch/orgextmapping/delete',
						data : row,
						success : function(data){
							if(data == 'success'){
